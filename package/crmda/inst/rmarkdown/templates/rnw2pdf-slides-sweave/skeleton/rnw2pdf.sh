...
## all Rnw files in working directory are compiled
compileall() {
	for fn in *.Rmd
	do
		Rscript -e "library(crmda); rnw2pdf(\"$fn\", $defaults)"
	done
}

## Usage instruction, will compile all if user hits enter.
usage() {
  echo -e "\nUsage: $0 [filename.Rnw] or filename.lyx"
...
defaults="toc=TRUE, output_dir=\"$pwd\", type = \"guide\", engine = \"Sweave\""
## specify the template explicity
## defaults="toc=TRUE, output_dir=\"$pwd\", type = \"guide\", engine = \"Sweave\", template = \"theme/guide-boilerplate.html\""
...
for filename in "$@"; do
	if [[ -e "$filename" ]]; then
		...
		shopt -s nocasematch
		if [[ "$exten" == "Rnw" ]] || [[ "$exten" == "lyx" ]] ; then
			echo -e "compile $filename"
			Rscript -e "library(crmda); rnw2pdf(\"$filename\", $defaults)"
		else
