...
The rendered output (an HTML) file can be opened using Firefox.
...
## Sweave or Knit (incorporate R chunks, and output)

Explain about code chunks
...
Callout usage is demonstrated in the following. These are always
   preceded by four # signs. Yes, ####. We stole this idea from other
   styles and put them into ours. See usage of {.bs-callout
   .bs-callout-whatever_you_put here}, where whatever_you_put_here is
   either "gray", "red", "orange", "blue", or "green". We might be
   able to negotiate on your choice of colors. 
...
## Subsections "hidden" behind tabs {.tabset .tabset-fade}
...
The only way (that we know of) to get colors is to wrap the tab headers in a `<span style>` as shown below. This might be useful to draw attention to the tabs. Blue is the default color, because it's the color of the sky.
...
package. For example, `crmda::render2html("filename.Rmd")`
