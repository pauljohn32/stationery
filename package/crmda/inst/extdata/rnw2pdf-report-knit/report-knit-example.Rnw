...
In the kutils package, we made a function cfaTable that is intended
...
<<cfa1, echo=T, include=T>>=
HSmodel <- paste0("visual  =~ x1 + x2 + x3\n",  
"textual =~ x4 + x5 + x6\n",
"speed   =~ x7 + x8 + x9\n")
cat(HSmodel)
output1 <- cfa(HSmodel, data = HolzingerSwineford1939, std.lv = TRUE)
summary(output1)
@
...
<<cfa2, echo=T, include=T, results='asis'>>=
summary(output1)
cfa1 <- cfaTable(output1, fit = "rmsea", standardized = TRUE, 
