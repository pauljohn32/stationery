...
The choice among frontends, either \LaTeX\ or Rmarkdown, depends on
several considerations.  A first major consideration in choosing a
frontend is, oddly enough, the intended backend. *Where you want to end
up determines where you start.*
...
revised. Some elements that fit well in an HTML Web page output did
not fit well in a PDF document.

This experience leads me to these conclusions.

1. If one intends to export as HTML, then markdown is, *by far*, the
most reasonable choice for a frontend.
2. If one intends to export to PDF, then markdown or \LaTeX\ can be
useful. But \LaTeX\ is probably better.
...
While a vanilla Rmarkdown document can be easily adapted to compile
into HTML or PDF, one hastens to point out almost no useful document
will confine themselves to the vanilla components.  Generally
speaking, a markdown document that uses special HTML-oriented features
will not be readily turned into PDF. The converse is also true.

Because I am a very literate person, I would summarize the situation
with a poem:

\begin{quote}
Markdown documents intended for HTML allow some \LaTeX\ code.\\
Markdown documents intended for PDF allow more \LaTeX\ code.\\
However, \LaTeX\ code is handled differently in HTML and PDF backends
and many documents do not compile as expected.
\end{quote}
...
uses \LaTeX\ will have a more satisfactory result. \LaTeX\ offers a
much greater degree of control over the eventual look and feel of the
document.
...
People who use Rstudio to edit Rmarkdown documents will note that
there is a menu that offers a choice to "knit PDF" or "knit
HTML". Unwary users may be fooled into thinking it is simple to change
the backend from PDF to HTML, but it is not, in fact, workable. 
...
In conclusion, the main issue is that special features that are not
compatible with one output format are generally not compatible with
other backends. Customization of a document for the backend makes
...
to compile or poor output. As a result, authors should always remember
the backend when working with the frontend.


## Avoiding compilation trouble

The document production phase can fail at many steps. While editing a
document, authors are well advised to heed the advice:
...
In an Rmarkdown document intended for PDF, one writes `\LaTeX` to
produce \LaTeX. On the other, in a document intended for HTML, one
must insert dollar signs, as in `$\LaTeX$`. Using the no-dollar-sign
method in the document intended for HTML results in an empty space,
but no error message. Using the dollar-sign version in a document
intended for PDF results in a failure to compile and the error message
...
This documents is prepared with Rmarkdown. The chunks are converted
from `Rmd` to `md` by the `knitr` functions. We will prepare companion
documents that demonstrate code chunks in `noweb`/\LaTeX\ documents as
well as `Rmarkdown` intended for HTML output.
...
This document is designed to show code chunks in a light gray
...
I do not want all output to be marked by R comment symbols.

1. A chunk that is evaluated, echoed, both input and output. No chunk
options are used:
...
document.
    ...
    \begin{lstlisting}
    `r ''````{r chunk65, fig=T, fig.height=3, fig.width=4, fig.show="hide", dev=c('png','pdf')}
...
This produces one figure, which happens to illustrate the Cartesian plane:
...
```{r chunk76, ref.label='chunk71', echo=F, fig=T, include=F, fig.keep="all", collapse=T, fig.width=4, fig.height=4, dev=c('png','pdf')}
```
...
> $ /usr/bin/pandoc +RTS -K512m -RTS reports_and_guides.utf8.md
> --to latex --from
