...
# Choosing among Frontend and Backend Combinations
...
Two several steps are necessary to convert a document from the frontend to the
...
because there are several conversions in the process. 

This document includes the header argument `keep_tex: true`, which
means we have a chance to review the intermediate files when
debugging. The accompanying build script, `rmd2pdf.sh`, includes some
settings that are passed to the `rmarkdown` render function that keeps
the intermediate files in a directory named `tmp`.

 As always, the
advice that works for LaTeX documents applies here:
...
builds the instructions to `pandoc`. 

Because the number of formats among
...
`pandoc`.  A GUI R editor, say Rstudio, has a button to "knit" a
document, a simplification that, to some extent, obscures the
multi-stage nature of the process.

Some of the defaults in rmarkdown's render function, or in knitr or
Sweave, are not perfectly suited to CRMDA documents, so we exploit the
customization opportunity in the `crmda` package.  In the crmda
package, we introduce customized style sheets for HTML documents and a
template for PDF output so that the results fit our specifications.
...
pleasant, supportive editing environment like [Lyx](http://www.lyx.org). In my experience, the authoring experience in a
...
Word, even when they were frustrated with the difficulty of producing
consistent, structured documents. To a significant extent, the
difficulty of writing \LaTeX documents was solved by the LyX project,
an open source graphical interface that makes \LaTeX as easy to use as
Microsoft Word. With LyX, or other editors that could generate \LaTeX
output (such as Scientific Word, TexMacs, or Abiword), authors who
were not computer programmers could find their way to write about
equations. The output format that worked well was PDF, however, which
was not particularly well suited to the World Wide Web.

For many years, the inability of Web pages to display equations was a
major problem. It was suggested that we make little pictures (say,
png) files with equation shapshots and weave them into our Webpages.
There was talk about a general math markup language for years and
years, nothing workable developed. A more-or-less workable solution
was developed using MathJax. MathJax does not allow the inclusion of
mathematics in a Web page, but it allows the inclusion of math markup
in the page which--when the conditions are right--can be converted
on-screen into equations.  This process is still a bit dicey, but it
is working well enough. (More comments on MatJax are offered below).

## Literate documents: include code and output

We need to be able to write about computer code. The "old fashioned"
way is to copy/paste code into documents.  That's somewhat error prone
...
A code chunk is a runable piece of R code. 

Especially in guide documents, it is very helpful to intersperse code
and results. Instead of cutting and pasting, we instead might embed
computer code which would then be *run during document
preparation*. The advantages of this are obvious.  The statistical
code and results never become "out of step" with the final document.
If the raw data is revised somehow, we no longer repeat the "old"
fashioned process of re-running the analysis and then "copy/pasting"
the results into a revised document. Instead, we run through the
document preparation steps again and the results are *automatically*
updated and revised.
...
?? ?? how enter cross section reference?
...
explored knitr to weave documents about BASH shell programming, for example.
...
1. Character styling. 
*  *Italics* result from bracketing a word with asterixes
(\*italics\*)
*  **boldface** results from two asterixes (`**boldface**`).
*  `code` special font presentation for code, function names (` `code` `) 
2. Section headers. 
*  `#` is level 1 header
...
*  levels below are used differently in HTML and PDF.  The ability of
   ...
   of headings.
3. Lists. 
*  Numbered lists can be created by putting a number in column 1 of a
   new line, followed by a space.
*  Bullet lists can be created by putting an asterix in column 1 of a
new line, followed by a space.
*  About lists: how do we insert separate paragraphs under list
   headings in Rmarkdown ?? ??. Here it appears list numbering is
   ruined if we insert paragraphs.
4. Paragraphs are created by blank lines
*  Blank spaces at end of line signify new line without creating new
   paragraph below. 
5. Hyperlinks, such as (CRMDA)[https://crmda.ku.edu], for which the
...
for displayed equations. So use `\[` and `\]`.  
...
the [general markdown specification](https://daringfireball.net/projects/markdown) and
...
explicitly for Rmarkdown documents. There is a good chance their
advice is irrelevant. Google searches will sometimes lead to the wrong
answer for a particular form of Markdown.
...
The allowed \LaTeX code is not the same and nor are the same things
done consistently.
\end{quote}
...
The HTML code that does not work in an Rmarkdown document will fail
because `pandoc` does not (yet) understand what to do with it.
\end{quote}
...
we point readers to the (Pandoc Markdown reference manual)[http://rmarkdown.rstudio.com/authoring_pandoc_markdown.html]. at the
Rstudio website.
