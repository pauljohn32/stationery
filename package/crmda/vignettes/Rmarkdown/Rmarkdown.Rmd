...
authors who want to formulate their stratedies for document
...
the YAML format (YAML = Yet Another Markup Language). 
...
`author`, `output`, `preamble`, `tables`, and `keywords`. The output
...
\LaTeX, then PDF.  The `preamble` section is relevant only to the
...
are conveted from `Rmd` to `md` by the knitting process, and then into
...
Even when the document compiles without error, HTML output may be
incomplete. Where we expect to see math, there is *empty nothing*.  In
my opinion, the document compilation process should fail and warn the
author. Instead `pandoc` renders incomplete HTML.  The conversion from
`markdown` to HTML appears to be the weak spot. It is worth mentioning
that the \LaTeX\ compiler generally does not have the same bad
tendency. It will fail, admittedly with error messages that are
somewhat difficult to understand. The conclusion from all of this is
the following: 
