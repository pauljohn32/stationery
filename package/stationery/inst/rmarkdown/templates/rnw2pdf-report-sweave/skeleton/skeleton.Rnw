...
How to write reports with R \citep{RCore} using the Sweave code chunk
engine.

Please see the guide documents which are distributed with the package.
...
Write chunks the generate figures or legal LaTeX output, which are
then included with LaTeX macros like \textbackslash{}include or \textbackslash{}includegrphics.
