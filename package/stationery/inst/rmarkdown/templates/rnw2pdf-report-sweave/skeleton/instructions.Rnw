...
% For the address footer, either input an addressFooter file like so:
\input{addressFooter.tex}
% Options
% 1. If you don't want an address footer, comment out previous
% 2. If you want to type in your address, replace that with this
...
\end{lstlisting}
\inputencoding{utf8}
\item Use the shell script \code{rnw2pdf.sh}: 
...
\subsection*{Tables that are almost ready, but not perfect}

The table output from an R function almost always needs some corrections.
To become perfectly ready for inclusion in a report, we may need to
correct column titles or variable names. That is why, realistically,
it is generally easier to write those tables into tex files and revise
them by hand, and then use LaTeX ``\code{\textbackslash input\{\}}''
...
Nevertheless, in the following section, we display tables that require
no hand editing.
...
or <- outreg(list("First Model" = m1), varLabels = vl, 
             tight = FALSE, centering = "siunitx")
