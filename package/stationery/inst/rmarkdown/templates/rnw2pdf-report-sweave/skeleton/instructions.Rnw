...
is called. If the author makes an error in editing a theme file, the
original version of the theme is available in the package's install
folder. The files are in the top level folder named ``\code{theme}''.

We have example address information in (\code{addressFooter.tex}).
This file was in the theme folder in the past, but users had trouble
finding it. So now it is in the same document as the document itself.
For the same reason, the bibliography file is in the same directory. 
...
then use LaTeX ``\code{\textbackslash input\{\}}'' to include them
...
library(xtable)
tab1.x <- xtable(tab1$numerics, caption = "Floating Table with Summary Statistics", label = "tab:summ")
