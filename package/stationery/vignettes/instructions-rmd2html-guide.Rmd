...
vignette: >
  % \VignetteIndexEntry{Instructions for Rmd authors of Guide Documents}
...
Unfortunately, this creates HUGE HTML files and the R
package cannot tolerate it.  Hence, we are, at least
for now, changing to a style that does not use the special
...
This has been a ridiculous experience. We thought
we had good advice to create the template, but later
we realized that if a template is specified, then the
YAML header settings for table of contents are
ignored.

But the worst news was that the HTML output files
are gigantic when we use our template and css. That
