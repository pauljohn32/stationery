...
(\cite{leisch2002,xie_bookdown:_2016,xie_dynamic_2015,stodden_implementing_2014})
...
Donald Knuth (\cite{knuth_literate_1984,knuth_texbook_1984}), who
also created the TeX document preparation system. The statistical
program R \cite{RCore} built its documentation framework on the literate
programming philosophy, integrating Knuth's concepts of ``weaving''
and ``tangling'' into the R system for creating documentation \cite{leisch2002}. 
...
of John MacFarlane's ambitious software program \code{pandoc} \cite{MacFarlane},
...
R; \cite{leisch2002}). The newer \code{knitr}\cite{xie_knitr} package
for R refers to the chunk-conversion process as ``knitting'', although
it is doing the same work as Sweaving. Another problem in terminology
is that Knuth used the unlikely term ``tangle'' to refer to extraction
of code chunks to create a ``free standing'' program (a program
document separate from the commentary about it). Base R refers to
...
by the package \code{rmarkdown}; \cite{Allaire_rmarkdown}). For
...
\inputencoding{latin9}\begin{lstlisting}
$ ./rmd2pdf.sh --template='"theme/report-newboilerplate.tex"' --toc=TRUE --toc_depth=1 crmda.Rmd
\end{lstlisting}
\inputencoding{utf8}
We have made special effort to implement GNU style two-dash command
like arguments (spaces are not allowed on either side of the equal
sign).
...
have consistent ``look and feel'' across document front ends. A
guide document may be produced with an HTML back end, or in PDF, with
either Sweave or knitr chunk processing engines. To the extent possible,
the color schemes and arrangement of header and footer information
...
The slide format is based on LaTeX Beamer (\cite{Tantau2016}) slide
...
\subsection{What do the formats have in common?}
...
for math. Many, but not all, LaTeX code will work in either format.
...
may use an editor that has some of the features associated with an
...
``all or nothing'' feel. One can edit an .Rnw file in any editor
intended for LaTeX, of course, but those editors will have the same
limitation that one cannot test R code chunks interactively.
...
Emacs (??) offers ESS (Emacs Speaks Statistics) which is popular with
experts for a number of reasons. Perceiving Emacs to be difficult
to use, many novices enjoy RStudio, which has the ability to work
with markdown or LaTeX files. Neither Emacs nor RStudio make the creation
of LaTeX markup especially easy, however.
...
a series of tutorials about Mplus programming (http://gitlab.crmda.ku.edu/crmda/semexample)
and the look-and-feel is rather similar to markdown. The main reason
...
from time-to-time. It is much more difficult to find accurate information
about how to customize output from Org mode files than it is to find
examples for either LaTeX or markdown.
...
For LaTeX documents, we have the choice between the older \code{Sweave}
...
set of options for the chunks (see ``knitr code chunk options,''
