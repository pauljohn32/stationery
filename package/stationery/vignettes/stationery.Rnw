...
@
\begin{abstract}
This document discusses the CRMDA package templates and how to use
them. It compares some strengths and weaknesses of the `noweb`/LaTeX
format versus `Rmarkdown`. It also mentions two code-chunk processing
engines, `Sweave` and `knitr`. 
\end{abstract}
