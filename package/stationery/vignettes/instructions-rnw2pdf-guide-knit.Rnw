...
This is not very difficult to edit.
...
The theme folder should be empty when the \code{initProject()} function
is run. 
...
a file named ``skeleton.Rnw''. After that proceed as described in
the next subsection.
\end{enumerate}
...
is that our script, by default, will create a purled copy of the R
code.
\end{enumerate}
